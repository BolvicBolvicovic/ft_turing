...
module Make : functor (I: INPUT) -> Machine = functor (I: INPUT) -> struct
        ...
        let process str_input state head =
                let updated_input =
                        if head = String.length str_input then
                                str_input ^ blank
                        else if head = (-1) then
                                blank ^ str_input
                        else str_input in
                ...
                let new_head = match action with
                        ...
                        | _ -> -1
                in
                let new_str = String.fold_left (fun acc element -> acc @ if List.length acc = head then [write.[0]] else [element]) [] updated_input in
                let new_str = String.of_seq (List.to_seq new_str) in
                let formated_state = 
                        if new_state = "HALT" then 
                                "\027[0;32mHALT\027[0;37m"
                        else if new_state = "ERROR" then
                                "\027[0;31mERROR\027[0;37m"
                        else new_state
                in
        ...
        let compile str_input = 
                if String.for_all (fun c -> List.exists (fun str -> str.[0] = c) alphabet) str_input then 
                        ...
                        let machine_alphabet = List.fold_left (fun acc str -> acc ^ str) "" alphabet in
                        let nb_of_states = List.length states - List.length finals in
                        if not (String.for_all (fun c -> String.exists (fun a -> a = c) utm_allowed_alphabet) machine_alphabet) then
                                invalid_arg "Error: alphabet is not compatible with the utm allowed alphabet: 0123456789abcdef+=-"
                                ...
                                let tail_states = List.filter (fun str -> not ((List.exists (fun s -> s = str) finals) || str = initial )) states in
                                let ht_state = StateHashtbl.create nb_of_states in
                                StateHashtbl.add ht_state initial (String.make 1 utm_allowed_alphabet.[0]);
                                List.iteri (fun i str -> StateHashtbl.add ht_state str (String.make 1 utm_allowed_alphabet.[i + 1])) tail_states;
                                let build_transition arr = 
                                        let raw_transitions = Array.fold_left (fun acc (read, to_state, write, action) -> 
                                                acc ^ read 
                                                ^ (
                                                        try
                                                                StateHashtbl.find ht_state to_state
                                                        with e -> 
                                                                if List.exists (fun str -> str = to_state) finals then "H" else raise e
                                                ) 
                                                ^ write ^ if action = "RIGHT" then "0," else "1,"
                                        ) "" arr in String.sub raw_transitions 0 (String.length raw_transitions - 1)
                                in
                                let first_state = StateHashtbl.find ht_state initial ^ "(" ^ build_transition (StateHashtbl.find transitions initial) ^ ")" in
                                print_string (machine_alphabet 
                                ^ "_" 
                                ^ List.fold_left 
...
let from_input json_path =
        if not (String.ends_with ~suffix:".json" json_path) then invalid_arg ("Error: " ^ json_path ^ " is not a JSON file.") else
        let open Core in
        let open Yojson.Basic.Util in
        let json = Yojson.Basic.from_file json_path in
        let name: string = json |> member "name" |> to_string in
        let alphabet: string list = json |> member "alphabet" |> to_list |> filter_string in
        let blank: string = json |> member "blank" |> to_string in
        let states: string list = json |> member "states" |> to_list |> filter_string in
        let initial: string = json |> member "initial" |> to_string in
        let finals: string list = json |> member "finals" |> to_list |> filter_string in
        ...
        transitions |> List.iter ~f:(fun (state, transitions) ->
                let transition_array =
                        if not (List.exists states ~f:(fun str -> String.equal str state)) then
                                raise (Invalid_argument ("Error with JSON file: " ^ state ^ " is not in states."))
                        else transitions |> to_list
                        |> List.map ~f:(fun t ->
                                ...
                                if not (List.exists alphabet ~f:(fun str -> String.equal str read)) then
                                        raise (Invalid_argument ("Error with JSON file: in " ^ state ^ ", read: '" ^ read ^ "' is not in alphabet."))
                                else if not (List.exists alphabet ~f:(fun str -> String.equal str write)) then
                                        raise (Invalid_argument ("Error with JSON file: in " ^ state ^ ", write: '" ^ write ^ "' is not in alphabet."))
                                else if not (List.exists states ~f:(fun str -> String.equal str to_state)) then
                                        raise (Invalid_argument ("Error with JSON file: in " ^ state ^ ", to_state: '" ^ to_state ^ "' is not in states."))
                                else if not (String.equal action "LEFT" || String.equal action "RIGHT") then
                                        raise (Invalid_argument ("Error with JSON file: in " ^ state ^ ", action: '" ^ action ^ "' is not a valid action."))
                                else
