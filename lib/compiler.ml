              ...
              (fun acc (line, _) ->
                ...
                | Inputs str ->
                    if str = "any" then acc
                      ...
                      String.fold_left (fun acc c -> c :: acc) acc str
                | _ -> acc)
              [] func.definition
          in
          let filtered_transitions =
            List.filter
              (fun (transition_array) ->
                let read_char = snd transition_array.(0) in (* "read" field *)
                let read_char_as_char = if String.length read_char = 1 then read_char.[0] else ' ' in
                (* Keep transition if it's not generated by "any" OR if it doesn't conflict *)
                not (List.mem read_char_as_char explicit_transitions) ||
                (* Check if this transition came from an explicit definition *)
                List.exists
                  (fun (line, _) ->
                    match line.inputs with
                    | Inputs str when str <> "any" ->
                        String.contains str read_char_as_char
                    | _ -> false)
                  func.definition)
              initial_transitions
          in
          let used_inputs =
           ...
           let error_transitions =
             List.fold_left
               ...
               [] filtered_alphabet
           in
           [filtered_transitions; error_transitions]) ))
    as_funcs
