...
let print_help () =
        ...
        print_endline "    --help -h                            -> prints this message"

let () =
        try
                match Array.length Sys.argv with
                        | 2 -> begin
                                if Sys.argv.(1) = "--help" || Sys.argv.(1) = "-h" then print_help () else invalid_arg "USAGE: ft_turing [OPTION] machine_name.json 'input_to_machine'"
                        end
                        | 3 -> begin 
                                if Sys.argv.(1) = "-c" && String.ends_with ~suffix:".s" Sys.argv.(2) then Compiler.compile Sys.argv.(2)
